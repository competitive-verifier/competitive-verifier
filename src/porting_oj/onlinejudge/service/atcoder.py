...
class AtCoderService(onlinejudge.type.Service):
    ...
    @classmethod
    def _list_cloned_branches(cls) -> List[str]: ...
...
class AtCoderProblem(onlinejudge.type.Problem):
    ...
    def _list_dropbox_folder(
        self, *, path: str, shared_link_url: str, session: requests.Session
    ) -> List[Dict[str, str]]:
        """
        :raises requests.exceptions.HTTPError: if no such problem exists
        ...
        while True:
            resp = utils.request(
                "POST", url, json=payload, session=session, raise_for_status=False
            )
            try:
                resp.raise_for_status()
            except:
    ...
    def _get_dropbox_shared_link_file(
        self, *, path: str, shared_link_url: str, session: requests.Session
    ) -> bytes:
        """
        :raises requests.exceptions.HTTPError: if no such problem exists
        ...
        resp = utils.request(
            "POST", url, headers=headers, session=session, raise_for_status=False
        )
        try:
            resp.raise_for_status()
        except:
    ...
    def download_system_cases(
        ...
        session = session or utils.get_default_session()
        shared_link_url = (
            "https://www.dropbox.com/sh/nx3tnilzqz7df8a/AAAYlTq2tiEHl5hsESw6-yfLa"
        )

        # list folders
        path = ""
        contest_folders = self._list_dropbox_folder(
            path=path, shared_link_url=shared_link_url, session=session
        )
        ...
        for entry in contest_folders:
            ...
            if self._match_system_cases_contest_folder(
                contest_folder_name=entry["name"]
            ):
                if contest_folder is not None:
                    raise SampleParseError(
                        "two folders match the contest: {} and {}".format(
        ...
        problem_folders = self._list_dropbox_folder(
            path=path, shared_link_url=shared_link_url, session=session
        )
        ...
        for entry in problem_folders:
            logger.debug("entry: %s", entry)
            if self._match_system_cases_problem_folder(
                contest_folder_name=contest_folder["name"],
                problem_folder_name=entry["name"],
            ):
                if problem_folder is not None:
                    raise SampleParseError(
                        "two folders match the problem: {} and {}".format(
        ...
        in_files = self._list_dropbox_folder(
            path=path, shared_link_url=shared_link_url, session=session
        )
        ...
        for entry in in_files:
            logger.debug("entry: %s", entry)
            path_in = "/{}/{}/in/{}".format(
                contest_folder["name"], problem_folder["name"], entry["name"]
            )
            data_in = self._get_dropbox_shared_link_file(
                path=path_in, shared_link_url=shared_link_url, session=session
            )
            path_out = "/{}/{}/out/{}".format(
                contest_folder["name"], problem_folder["name"], entry["name"]
            )
            data_out = self._get_dropbox_shared_link_file(
                path=path_out, shared_link_url=shared_link_url, session=session
            )
            testcases.append(
                TestCase(
                    entry["name"],
