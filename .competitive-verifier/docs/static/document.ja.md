...
互換性のため、C++ では `#define PROBLEM "https://judge.yosupo.jp/problem/aplusb"` のようにマクロで属性を定義できます。しかし、これを列挙するためにコンパイルが必要で `oj-resolve` の実行が遅くなる原因となります。マクロによる属性定義は避けて、上記のように `read_macros = false` を設定することを推奨します。
...
`documentation_of`  文字列は相対パスか絶対パスを受け付けます[^relative_path].
...
[^relative_path]: `./` あるいは `..` から始まる場合は当該ファイルのパスからの相対パスであると認識されます。また、`//` から始まる場合は リポジトリルートからの絶対パスであると認識されます。ディレクトリ区切り文字には `/` を使い、大文字小文字を正しく入力してください。
